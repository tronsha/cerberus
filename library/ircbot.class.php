...
class IRCBot
{
    ...
    public function __construct($config = NULL)
    {
        ...
        $this->config['autorejoin']        = is_array($config) && isset($config['bot']['autorejoin'])   ? ($config['bot']['autorejoin']   == 1 ? true : false) : false;

        $this->config['logfile']['error']  = is_array($config) && isset($config['log']['error'])        ? ($config['log']['error']        == 1 ? true : false) : true;
        $this->config['logfile']['socket'] = is_array($config) && isset($config['log']['socket'])       ? ($config['log']['socket']       == 1 ? true : false) : false;
        $this->config['logfile']['sql']    = is_array($config) && isset($config['log']['sql'])          ? ($config['log']['sql']          == 1 ? true : false) : false;
        $this->config['dailylogfile']      = is_array($config) && isset($config['log']['dailylogfile']) ? ($config['log']['dailylogfile'] == 1 ? true : false) : true;
    }
    ...
    public function setNick($nick = NULL)
    {
        ...
        $this->var['me'] = $nick;
        if($this->init === true)
            $this->sql_query('UPDATE `bot` SET `nick` = "'.$this->db->escape_string($this->bot['nick']).'" WHERE `id` = "'.$this->bot['id'].'"');
        return $this;
    }
    ...
    public function init()
    {
        ...
        $this->db_connect();
        $this->sql_query('INSERT INTO `bot` SET `pid` = "'.$this->bot['pid'].'", `start` = NOW(), `nick` = "'.$this->db->escape_string($this->bot['nick']).'"');
    ...
    public static function randomNick()
    {
        ...
        $vokal = 'aeiou';
        $nick = '';
        for($i = 0; $i < 3; $i++) $nick .= $konsonant{rand(0,20)}.$vokal{rand(0,4)};
        return ucfirst($nick);
    }
    ...
    protected function getServer($i = 0)
    {
        ...
        $this->sql_query('UPDATE `bot` SET `server_id` = "'.$this->server['id'].'" WHERE `id` = "'.$this->bot['id'].'"');
    }
    ...
    {
        $query = $this->sql_query('SELECT `text` FROM `preform` WHERE `network` = "'.$this->server['network'].'" ORDER BY `priority` DESC');
        while($copy = $this->db->fetch_assoc($query))
            $this->sql_query('INSERT INTO `write` SET `text` = "'.$this->db->escape_string($copy['text']).'", `bot_id` = "'.$this->bot['id'].'"');
            ...
            if($send['text'] != '')
            {
                ...
                foreach($array[1] as $value) 
                    if(array_key_exists($value, $this->var))
                        $send['text'] = preg_replace('/%'.$value.'(\s|$)/i', $this->var[$value]."\\1", $send['text']);
    ...
    protected function run()
    {
        while(!feof($this->fp))
        {
            ...
            unset($input);
            msleep(8);
            if((time() - $this->lastping) > 600) break;
            if($this->run === false) return true;
        }
    ...
    {
        list($dummy, $channel, $anz_user) = split(' ', $rest);
        ...
        if(preg_match("/\x01([A-Z]+)( [0-9]+)?\x01/i", $text, $matches))
        {
            switch($matches[1])
            {
                ...
                case 'TIME':
                    ...
                    break;
                case 'FINGER':
                    $send = 'FINGER '.$this->config['info']['name'].' ('.$this->config['info']['homepage'].') Idle '.round(getmicrotime() - $this->time['irc_connect']).' seconds';
                    break;
            }
    ...
    protected function onNick($nick, $text)
    {
        ...
        $sql = 'UPDATE `channel_user` SET `user` = "'.$this->db->escape_string($text).'" WHERE `bot_id` = "'.$this->bot['id'].'" AND `user` = "'.$this->db->escape_string($nick).'"';
        $this->sql_query($sql);
    }

    protected function on353($rest, $text)
    {
        list($me, $dummy, $channel) = split(' ', $rest);
        $user_array = split(' ', $text);
        foreach($user_array as $user)
        {
            preg_match("/^([\+\@])?([^\+\@]+)$/i", $user, $matches);
            $this->sql_query('INSERT INTO `channel_user` SET `user` = "'.$this->db->escape_string($matches[2]).'", `mode` = "'.$this->db->escape_string($matches[1]).'", `channel` = "'.$this->db->escape_string($channel).'", `bot_id` = "'.$this->bot['id'].'"');
        }
    }

    protected function on332($rest, $text)
    {
        list($me, $channel) = split(' ', $rest);
        $this->onTopic($channel, $text);
    }

    protected function onTopic($channel, $topic)
    {
        $sql = 'UPDATE `channel` SET `topic` = "'.$this->db->escape_string($topic).'" WHERE `bot_id` = "'.$this->bot['id'].'" AND `channel` = "'.$this->db->escape_string($channel).'"';
            $this->sql_query($sql);
    }
    ...
    protected function onJoin($nick, $channel)
    {
        if($nick == $this->var['me'])
        {
            $this->sql_query('INSERT INTO `channel` SET `channel` = "'.$this->db->escape_string($channel).'", `bot_id` = "'.$this->bot['id'].'"');
            $this->mode($channel);
        }
        else
        {
            $this->sql_query('INSERT INTO `channel_user` SET `user` = "'.$this->db->escape_string($nick).'", `channel` = "'.$this->db->escape_string($channel).'", `bot_id` = "'.$this->bot['id'].'"');
        }
    }

    protected function onKick($bouncer, $rest)
    {
        list($channel, $nick) = split(' ', $rest);
    ...
    protected function onPart($nick, $channel)
    {
        if($nick == $this->var['me'])
        {
            $this->sql_query('DELETE FROM `channel` WHERE `channel` = "'.$this->db->escape_string($channel).'" AND `bot_id` = "'.$this->bot['id'].'"'); 
            $this->sql_query('DELETE FROM `channel_user` WHERE `channel` = "'.$this->db->escape_string($channel).'" AND `bot_id` = "'.$this->bot['id'].'"');
        }
        else
        {
            $this->sql_query('DELETE FROM `channel_user` WHERE `user` = "'.$this->db->escape_string($nick).'" AND `channel` = "'.$this->db->escape_string($channel).'" AND `bot_id` = "'.$this->bot['id'].'"');
        }
    }
  
    protected function onQuit($nick)
    {
        $this->sql_query('DELETE FROM `channel_user` WHERE `user` = "'.$this->db->escape_string($nick).'" AND `bot_id` = "'.$this->bot['id'].'"');
    }
    ...
    {
        $this->sql_query('INSERT INTO `write` SET `text` = "'.$this->db->escape_string('PRIVMSG '.$to.' :'.$text).'", `bot_id` = "'.$this->bot['id'].'"');      
    }

    protected function notice($to, $text)
    {
        $this->sql_query('INSERT INTO `write` SET `text` = "'.$this->db->escape_string('NOTICE '.$to.' :'.$text).'", `bot_id` = "'.$this->bot['id'].'"');
    }
    
    protected function quit($text)
    {
        $this->sql_query('INSERT INTO `write` SET `text` = "'.$this->db->escape_string('QUIT :'.$text).'", `bot_id` = "'.$this->bot['id'].'"');
    }

    protected function mode($text = NULL)
    {
        $this->sql_query('INSERT INTO `write` SET `text` = "'.$this->db->escape_string('MODE'.($text === NULL ? '' : ' '.$text)).'", `bot_id` = "'.$this->bot['id'].'"');
    }
    
    protected function join($channel)
    {
        $this->sql_query('INSERT INTO `write` SET `text` = "'.$this->db->escape_string('JOIN '.$channel).'", `bot_id` = "'.$this->bot['id'].'"');
    }

    protected function channellist()
    {
        $this->sql_query('DELETE FROM `channellist` WHERE `network` = "'.$this->db->escape_string($this->server['network']).'"');
        $this->sql_query('INSERT INTO `write` SET `text` = "'.$this->db->escape_string('LIST').'", `bot_id` = "'.$this->bot['id'].'"');
    }
